...
- **`eigenstrat_fileset`**:
Prefix of the eigenstrat fileset (.geno, .snp, .ind files) containing all individuals you wish to cluster as well as those for qpadms reference populations. A vast collection of ancient and modern samples free to download in eigenstrat format can be found at the [Allen Ancient DNA Resource](https://reich.hms.harvard.edu/allen-ancient-dna-resource-aadr-downloadable-genotypes-present-day-and-ancient-dna-data) hosted by the David Reich Lab. A versatile tool for fileset manipulation (e.g. keeping a subset or merging with your own data) is [PLINK](https://www.cog-genomics.org/plink/). Converting from eigenstrat to plink format can be done with [convertf](https://github.com/argriffing/eigensoft/tree/master/CONVERTF). **CAUTION**: Do not have the specified 'eigenstrat_fileset' and its plink format (.bed, .bim, .fam files) in the same directory, otherwise admixtools2's `extract_f2` will attempt to use the plink format which does not have updated population labels and fail.
...
- **`periods_file`**: A tab separated text file where custom periods can be set upon which the clusters are split into periodic subclusters. First column is periods name, second column is older period boundary and third column is younger period boundary; in years BP. Period boundaries are inclusive, so avoid overlap! If not provided, defaults to clusSearch/.scripts/standard_periods, which divides 10000 to 0 BP into 500 year bins.
